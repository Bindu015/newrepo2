variable "account_principal_arns" {
  description = "ARNs that need access to resources created by this IaC such as KMS keys"
  type        = list(string)
}
...
variable "create_kms_key" {
  description = "Indicate whether to create a new KMS Key or use existing one"
  type        = bool
  default     = false
}

variable "component_short_name" {
  description = "Abbreviation of the component name (BitBucket repository name)"
  type        = string
  validation {
    condition     = length(var.component_short_name) >= 2 && can(regex("[a-z]+", var.component_short_name))
...
variable "deadletter_queue_name" {
  description = "The name of the SQS deadletter queue. It will automatically be prefixed with your project key to meet IAM guidelines."
  type        = string
}

variable "deployer_role" {
  description = "Name of the jenkins role passed as KMS key owner deployer role"
  type        = list(string)
}
...
variable "kms_key_purpose" {
  description = "The purpose of this KMS Key. For example, if you will just have one in your account, you can call it 'global' or 'account'. If you will have separate keys for certain apps, or services, you can provide a value of 'app1' or 's3' or even 'myapp-sns', etc. etc. This is useful for seeing not only what the key was made for, but for looking up its alias for using elsewhere."
  type        = string
  default     = ""
}
...
variable "topic_name" {
  description = "The name of the SNS topic. It will automatically be prefixed with your project key to meet IAM guidelines."
  type        = string
}
...
variable "daily-backup" {
  description = "The daily backup indicator"
  type        = string
  default     = "dailybackup1800"
  validation {
    condition     = can(regex("^(dailybackup1800|dailybackup2200|dailybackup0200|nobackup)$", var.daily-backup))
    error_message = "Must be one of the allowed values."
  }
}

variable "cloudwatch_threshold" {
  description = "Provide threshold value for GreaterThanThreshold CloudWatch alarm"
  type        = number
  default     = 100
}
...
locals {
  ...
  topic_canonical_name = "${var.project_key}_${var.topic_name}"
  topic_name_arn       = "arn:aws:sns:${data.aws_region.current.name}:${local.account_id}:${local.topic_canonical_name}"
  ...
  dlq_name             = "${var.project_key}_${var.deadletter_queue_name}"
  ...
  tags = merge({
    ...
    "tu:daily-backup"        = var.daily-backup
  }, var.custom_tags)
}